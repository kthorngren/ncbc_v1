...
class Competitions:
    ...
    def __init__(self):
        ...
        self._db = MySql(**CONN, db=DATABASE)
    ...
    def get_active_competition(self):
        ...
        logger.info(sql)
        result = self._db.run_sql(sql=sql, get='one')
    ...
    def get_comp_status(self):
        ...
        pkid = self.get_active_competition()

        sql = 'select count(*) as brewers from brewers where fk_competitions = "{}"'.format(pkid)
        result = self._db.run_sql(sql=sql, get='one')
        ...
        sql = 'select sum(fk_competitions = "{pkid}") as entries, sum(inventory = "1") as checked_in, ' \
              ...
              'from entries where fk_competitions = "{pkid}"'.format(pkid=pkid)
        result = self._db.run_sql(sql=sql, get='one')
        ...
        sql = 'select * from sessions where judging = "1" or setup = "1" and ' \
              'fk_competitions = "{}" '.format(pkid)
        sessions = self._db.run_sql(sql=sql, get='all')

        sql = 'select fk_sessions_list, is_judge from volunteers where fk_competitions = "{}"'.format(pkid)
        result = self._db.run_sql(sql=sql, get='all')
